...
|`GLOBAL_COUNTER`   |`LOG_ADDR` + atomic global counter|26.51/122/183/245^3^|17
|`BUFFER_MANAGER`   |`LOG_FULL` + integration with buffer manager|TODO|TODO
|`TSan`   |Instrumented with ThreadSanitizer|20.07|74
|`ngrt`   |Instrumented with RV-Predict[C]@`ad38cb5`|444.82|1166
|=======================
*^1^ Average CPU cycles spent per iteration.*

*^2^ Number of instructions of each iteration in the common case.*

*^3^ The four numbers correspond to the cases of one, two, three, and four logging threads, respectively.*
